...
class DirEntry {
	...
	public:
		DirEntry() {}
		DirEntry(const std::string& in_name, unsigned int in_inode) { m_name = in_name; m_inode = in_inode; }
		const std::string& name() { return m_name; }
		unsigned int inode() { return m_inode; }
};
...
class Inode {
	private:
		...
		std::vector<unsigned int> m_links; //Which directory inodes link to this inode
	...
	public:
		Inode() {}
		const std::vector<unsigned int>& blocks() { return m_blocks; }
		const std::vector<DirEntry>& dirEntries() { return m_dirEntries; }
		const std::vector<unsigned int>& links() { return m_links; }
